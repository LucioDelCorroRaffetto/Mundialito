...
| 2026-07-02 | Sesión 2 — Celebración + leaderboard animado | `feat/sesion-2-celebracion-leaderboard` (8d948bd) + fix `7d63a75`, mergeadas a main y deployadas | ✅ Validado en prod por el usuario: toast "¡Acertaste! +N" / "¡Exacto! +N" dispara correctamente. Bug encontrado y corregido: el confetti/glow estaba montado sobre la card "Tu pronóstico", que queda debajo del timeline de eventos en partidos live/finished — invisible sin scrollear. Se movió a la score card de arriba (siempre visible sin scroll, commit `7d63a75`). El reorder animado del leaderboard/standings y el count-up ya estaban implementados (LayoutGroup+layout en Row/LeaderboardRow, useCountUp en puntos/XP) — confirmado por revisión de código en Sesión 3, sin cambios adicionales necesarios. |
| 2026-07-02 | Sesión 3 — Evolución de posiciones + head-to-head | commit directo a main | ✅ Implementado: endpoint `GET /leagues/:id/standings/history` (agrupa por día AR, `lib/standings-history.ts` con 4 tests), gráfico SVG propio (`league-history-chart.tsx`, colapsable en `league-detail.tsx`, solo si ≥2 días), página `/h2h/:userIdA/:userIdB` (`head-to-head.tsx` + `h2h-row.tsx`, merge client-side de `useEnrichedUserPredictionHistory` ×2 sin endpoint nuevo), botón "Comparar" en perfil ajeno y ícono ⚔️ por fila de standings. `tsc -b`, eslint y toda la suite de tests (api+web) en verde. No se pudo verificar visualmente en el navegador porque el login local requiere OAuth (limitación conocida) — pendiente que el usuario lo valide en prod con una liga con partidos ya jugados. |
| 2026-07-03 | Sesión 4 — Reacciones por liga | `feat/sesion-4-reacciones-liga` (ffcf70f + fixes), mergeada a main | ✅ Implementado: schema `prediction_reactions`, `POST /predictions/:id/reactions` (toggle + 403 auto-reacción/no-miembro/no-revelado + push con throttle 10min), `GET /predictions/match/:matchId/reactions`, `isPredictionRevealed()` extraída a `lib/match-helpers.ts` y reusada en `match-predictions.ts`, hook `use-reactions.ts` + componente `reaction-row.tsx` (popover de 6 emojis, chips optimistas, solo lectura en pronósticos propios) integrado en `match-detail.tsx`. `tsc -b`/`eslint` (api+web) en verde, suite de tests de api en verde. **`npx drizzle-kit push` falla en este entorno** con `Cannot find module './users.js'` (esbuild-register interno de drizzle-kit no resuelve imports estilo NodeNext `./x.js`→`x.ts`; reproducido también en `main` sin cambios → bug de tooling, no del schema). Workaround: `npx tsx node_modules/drizzle-kit/bin.cjs push` (tsx sí resuelve la extensión) — dejar documentado para sesiones futuras. Al pushear así, drizzle-kit reportó además un drop de la columna `stage` en `matches` (104 filas) — investigado y confirmado legacy huérfana (`scripts/fix-schema.ts` ya había migrado sus datos a `round` tiempo atrás); se hizo backup de la tabla completa antes de aprobar el drop. DoD validado con curl contra Turso real usando 2 usuarios descartables (creados y borrados via SQL directo al terminar): toggle 200, reacción propia 403, no-miembro 403, no-revelado 403, GET agregado con `count`/`reactedByMe` correctos. Bug no relacionado encontrado y reportado aparte (no arreglado en esta sesión): `DELETE /auth/me` puede tirar 500 por expiración del stream interactivo de Turso cuando el borrado tarda mucho. |
| 2026-07-03 | Sesión 5 — Wrapped backend | `feat/sesion-5-wrapped-backend` (4914986) + export fix (31f42c5), mergeada a main y deployada | ✅ Implementado: `lib/wrapped.ts` puro (`longestStreak`, `bestHit` con fallback por goles totales ya que `prediction-aggregates.ts` no expone % de forecast por score, `nearestRival`, `mostPredictedTeam`) con 15 tests. Handler `GET /users/me/wrapped` (gate `TOURNAMENT_NOT_FINISHED` 409 con bypass `?preview=1` + admin, cache in-memory TTL 1h): reusa `dedupeBestPerMatch`/`computeLevel`/`computeUserXp`, calcula standings por liga (excluyendo la personal, mismo criterio que `leagueCount`), rank global (mismo patrón que `/users/leaderboard`), `championPick` contra el ganador de la final (`round='final'`), standings de fantasy y top-3 achievements por `xpReward`. `tsc -b`, eslint y suite completa de api (111 tests) en verde. Validado con curl contra prod: `/health` 200, gate 409 sin preview y con `preview=1` sin ser admin. No se pudo invocar el endpoint completo con la sesión real de admin (sin credenciales), así que se validó la lógica a mano vía Turso read-only (`.env.ro`) con un script descartable: `round='final'` está `scheduled` (gate correcto, torneo no terminó), y para el usuario real Luchi (id 2) el query de predicciones/puntos replicado a mano dio 84 predicciones finished, 118 puntos, 8 exactos, 46 correctos — coherente y sin inconsistencias. `buildWrapped` quedó exportado desde el handler para poder re-usar este tipo de script de validación a futuro. Bug preexistente sin resolver (ya reportado en Sesión 4): `DELETE /auth/me` tira 500; quedó un usuario de prueba descartable (id 115, sin datos) huérfano en prod por este bug, pendiente de limpieza o fix aparte. |
| 2026-07-03 | Sesión 6 — Wrapped frontend + share | commit directo a main (`85eef13`), deployada | ✅ Implementado: `useWrapped()` (retry:false, expone 409 `TOURNAMENT_NOT_FINISHED` como `notReady`), tipos `Wrapped*` en `types/api.ts`. Página `/wrapped` (`pages/wrapped.tsx`, ruta lazy **fuera** del grupo de `AppShell` con su propio `RequireAuth` — full-bleed sin tab-bar/sidebar, la opción que menos tocaba el router) con slides tipo stories: tap mitad derecha/izquierda + swipe (`drag="x"`) avanzan/retroceden, barra de progreso segmentada arriba, `AnimatePresence`+`scaleVariants`(`fadeVariants` si `reduced`), `useCountUp` en los números. Slides condicionales según qué trae el payload (bestHit/rival/mostPredictedTeam/championPick/fantasy/logros se saltean si vienen `null`/vacíos). Imagen compartible client-side (`wrapped-share-image.ts`): canvas 1080×1920 con `--accent` resuelto vía `getComputedStyle`, `navigator.share({files})` con fallback a `<a download>` si no hay Web Share con archivos (desktop). Banner en home (`wrapped-banner.tsx`, mismo patrón que `KnockoutPhaseBanner`) cuando `useTournamentPhase().kind === 'completed'`. Backend: tabla `worker_flags` (Drizzle, key/value/updatedAt) aplicada con el mismo workaround de Sesión 4 (`npx tsx node_modules/drizzle-kit/bin.cjs push`, sin prompts esta vez); job `send-wrapped-ready` en el worker (chequea `matches.round='final'` finished + flag idempotente, push a TODAS las suscripciones), enganchado en `run-daily.ts` después del recordatorio diario. `tsc --noEmit` (front + api + worker), `eslint` (0 errores, solo warnings preexistentes ajenos) y suite de tests de api (111) en verde; `npm run build` del front generó el chunk `wrapped-*.js` sin warnings nuevos. Validado en prod: push a `origin/main` confirmado por archivo (patrón del agente concurrente), Vercel sirve `/wrapped` (200) con el chunk `wrapped-ByMKzHpc.js` ya en el bundle (`Age: 13` en el `index.html` → deploy fresco), `/health` de Render 200. **No se pudo hacer el walkthrough real en el navegador** (banner → stories → compartir a WhatsApp) por la misma limitación de sesiones anteriores: el login requiere Google OAuth, sin credenciales disponibles en este entorno — pendiente que el usuario lo valide con `?preview=1` siendo admin antes del 19/7, como pide el DoD. Nada nuevo para `backfill:fifa` en esta sesión. |
| 2026-07-03 | Sesión 7 — Identidad: fuentes, logo, og:image | commit directo a main | ✅ Implementado: **og:image** (`scripts/generate-og.mjs`, sharp) — escudo + wordmark "Mundialito"/tagline/descripción/URL, todo el texto convertido a paths con opentype.js (evita depender de fuentes del sistema al rasterizar; `font.getPath()` de alto nivel revienta en Space Grotesk con "lookupType 6 not yet supported" — hubo que hacer el layout glyph-por-glyph con `charToGlyph`), `public/og-image.png` generado y meta `og:title/description/image/type/url` + `twitter:card/image` en `index.html`. **Fuentes**: `@fontsource/inter` (400-800), `@fontsource/space-grotesk` (400/500/700) y `@fontsource/russo-one` (400) importados en `main.tsx`; borrados los `<link>`/preconnect de Google Fonts; **Oswald eliminado** (`font-tagline` → `font-display` en `splash.tsx`, key `tagline` fuera de `tailwind.config.ts`); CSP de `vercel.json` sin `fonts.googleapis/gstatic` (agregado `font-src 'self'`). **Logo a paths**: en `logo.tsx` los 3 `<text>★</text>` pasaron a un `STAR_PATH` reutilizado (transform translate/scale) y "MUNDIALITO"/"2026" a paths generados con `scripts/text-to-path.mjs` (opentype.js + TTF de Russo One vía `@fontsource`); paths compartidos con los scripts de íconos/og en `scripts/shield-svg.mjs` para no triplicar la data. **Íconos regenerados** (`scripts/generate-icons.mjs`, sharp): `apple-touch-icon.svg/png` (180×180), `icons/icon-192.svg/png`, `icons/icon-512.svg/png`, `icons/icon-maskable.svg/png` (safe-zone intacta) — todos sin `font-family`. `favicon.svg` no se tocó (ya era un ícono minimalista sin `<text>`, nada que migrar). Verificado visualmente contra el logo anterior (git stash + screenshot zoomed lado a lado vía dev server local): estrellas y wordmark prácticamente idénticos. `tsc -b`, `eslint` (0 errores, solo warnings preexistentes ajenos) y suite de tests del front (48) en verde; `npm run build` OK. Peso de fuentes: 9 archivos woff2 latin self-hosted (Inter×5 + Space Grotesk×3 + Russo One×1) ≈164 KB, vs. el baseline remoto de 4 familias (Inter+Oswald+RussoOne+SpaceGrotesk, 11 archivos) — Oswald queda eliminado por completo y ya no hay requests a dominios de Google Fonts (confirmado por Network tab en el dev server). **No se corrió Lighthouse** (sin tooling de Chrome headless disponible en este entorno) ni se pudo probar el preview real de WhatsApp — pendiente que el usuario valide el link preview pegando la URL de prod. Pusheado a `main` (commit `6868ed7`) y validado en prod: `/og-image.png` 200, `index.html` sin referencias a `fonts.googleapis/gstatic`, header CSP en prod ya sin esos dominios. |
| 2026-07-03 | Sesión 8 — Backlog técnico menor | commit directo a main | ✅ Implementado: **(1) Lock `/sync`**: flag módulo `syncInProgress` en `app.ts`, 429 si ya está corriendo, `try/finally` para resetear — el handler se partió en `runSync()` standalone para poder envolverlo. **(2) Rate limit genérico**: `rateLimit({windowMs:60_000,max:300,routeKey:'api-general'})` montado antes de `apiRouter` en `/api/v1` (reusa `middleware/rate-limit.ts`). **(3) `--bg-elevated` claro**: `rgba(0,0,0,0.02)`→`0.04` en `palettes.ts`. **(4) Tinte de acento en `--bg-deep`** (único ítem a criterio): `color-mix(in srgb, #0a0e1a 96%, var(--accent))` en modo oscuro — verificado matemáticamente contra la tabla de ratios auditada (Sesión previa, 2026-06-21): peor caso (magenta, margen más chico) pasa de 5.71 a ~5.55 sobre el mínimo AA de 3.0 para UI, texto/texto-muted sin riesgo (mejoran levemente); no se pudo confirmar visualmente porque el puerto 5174 ya estaba tomado por el dev server de otra sesión concurrente en este mismo repo — mantenido por el cálculo, pendiente que el usuario lo mire en pantalla. **(5) `is_admin` en DB**: columna `users.is_admin` (integer default 0) agregada con `ALTER TABLE` directo vía script descartable — **`npx drizzle-kit push` propuso truncar la tabla `users` (91 filas)** al agregar la columna NOT NULL pese al `.default(0)` declarado en el schema (mismo tipo de fricción que Sesión 4 con `./users.js`); se abortó y se aplicó el ALTER a mano, sin pérdida de datos. `require-admin.ts` ahora es async: sigue aceptando `ADMIN_USER_IDS` (env, fallback/bootstrap) y además consulta `users.isAdmin` en DB. Nuevo `PATCH /admin/users/:id {isAdmin}` (`update-user-admin.ts`). **(6) Cookie httpOnly**: ya estaba documentado como descartado en el punto 6 de esta sesión (decisión de Sesión 1), sin acción adicional. **Ítem 7 (compactar reacciones) NO implementado** — quedó fuera de alcance por ser explícitamente "a criterio, solo si sobra tiempo"; sigue en el backlog de este documento para una sesión futura. `tsc -b` (front+api) 0 errores, `eslint` 0 errores (solo 8 warnings preexistentes ajenos), suite de tests 111 (api) + 48 (front) en verde. DoD validado con curl real contra un server local en puerto 3999 (3000 ocupado por el server de la sesión concurrente) + Turso de prod: dos `/sync` concurrentes → uno 200 y otro 429; burst de 302 req/min → 300 OK + 429 con `Retry-After`; toggle de admin end-to-end con 2 usuarios descartables (creados/borrados por SQL directo) — target sin admin 403 → admin lo promueve (200) → target ahora entra a una ruta admin (200, sin redeploy) → admin lo revierte (200) → target vuelve a 403. |
