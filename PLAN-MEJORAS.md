...
| 2026-07-02 | Sesión 4 — Reacciones por liga | `feat/sesion-4-reacciones-liga` (ffcf70f), sin mergear | 🔄 Código implementado completo: schema `prediction_reactions`, `POST /predictions/:id/reactions` (toggle + 403 auto-reacción/no-miembro/no-revelado + push con throttle 10min), `GET /predictions/match/:matchId/reactions`, `isPredictionRevealed()` extraída a `lib/match-helpers.ts` y reusada en `match-predictions.ts`, hook `use-reactions.ts` + componente `reaction-row.tsx` (popover de 6 emojis, chips optimistas, solo lectura en pronósticos propios) integrado en `match-detail.tsx`. `tsc -b` y `eslint` (api+web) en verde; suite de tests de api en verde (96 tests, sin tests nuevos propios — la spec solo pide DoD por curl). **Bloqueo:** `npx drizzle-kit push` falla en este entorno con `Cannot find module './users.js'` al resolver `achievements.ts` (reproducido también en `main` sin mis cambios → problema de tooling/Node, no del schema nuevo) — la tabla `prediction_reactions` NO se creó en Turso. Sin la tabla no se puede correr el DoD por curl contra prod. Pendiente: resolver el push (revisar versión de drizzle-kit/Node o correrlo desde otra máquina/CI) y validar DoD antes de mergear a main. |
