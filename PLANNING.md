...
| **D2** | Fixture 2026 ¿disponible en OpenFootball? | ✅ **Disponible** en `openfootball/worldcup.json` (nombre real del repo, sin guión). URL raw: `https://raw.githubusercontent.com/openfootball/worldcup.json/master/2026/worldcup.json`. Fuente TXT: `openfootball/worldcup`, paths `worldcup/2026--usa/cup.txt` (grupos) y `cup_finals.txt` (eliminatorias). Schema: `rounds[].matches[]` con `{date, time, team1{name,code}, team2{...}, group, ground/stadium}`. Dominio público, sin API key. En este entorno (sandbox sin red) no se pudo descargar el JSON para generar el seed real; queda pendiente ejecutar el fetch + parse en un entorno con red. Alternativas si el JSON faltase: fixturedownload.com (CSV/JSON), Kaggle "fifa-world-cup-2026-match-data", football-data.org (free tier, competition `WC`), API-Football. |
